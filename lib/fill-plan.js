...
// после всех "твёрдых" блоков, по одному, в порядке файла (нижняя половина
// двери раньше верхней и т.п.). water/lava сюда НЕ входят - см. LIQUID_BLOCK_NAMES
// ниже: они льются объёмами через /fill, а не по блоку (см. §4.4/§10.4.8 BOT.md).
...
// Жидкости идут отдельным, самым последним проходом (после всех остальных
// крепящихся блоков): в отличие от них, жидкости не требуют ориентации/
// точки крепления, поэтому их можно (и нужно, ради скорости - см. §10.4.8
// BOT.md) схлопывать в боксы через /fill, как твёрдые, а не лить по блоку.
// Идут последними, чтобы борта (стены, дно) уже стояли к моменту разлива.
const LIQUID_BLOCK_NAMES = new Set(['water', 'lava', 'flowing_water', 'flowing_lava']);

function isLiquidBlock(name) {
  const bare = name.startsWith('minecraft:') ? name.slice('minecraft:'.length) : name;
  return LIQUID_BLOCK_NAMES.has(bare);
}

// Блоки, подверженные гравитации: без твёрдой опоры снизу (в момент
// установки, а не только в итоговом состоянии) превращаются в падающую
// сущность - см. §10.4.8 BOT.md (песок в схемах 1b/1c падал именно из-за
// порядка боксов, не из-за итоговой геометрии).
const FALLING_BLOCK_NAMES = new Set(['sand', 'gravel', 'concrete_powder', 'anvil', 'dragon_egg']);

function isFallingBlock(name) {
  const bare = name.startsWith('minecraft:') ? name.slice('minecraft:'.length) : name;
  return FALLING_BLOCK_NAMES.has(bare);
}

/**
 * Делит схему на три группы: solid (мёржится в боксы, снизу вверх),
 * attached (крепящиеся блоки - идут по одному, как есть, в исходном порядке
 * файла) и liquid (вода/лава - тоже мёржатся в боксы, снизу вверх, но
 * отдельным финальным проходом, см. buildFillCommands). Порядок elements
...
function splitSchemaByAttachment(schema) {
  ...
  for (const entry of schema) {
    ...
    if (isLiquidBlock(name)) liquid.push(entry);
    else if (isAttachedBlock(name)) attached.push(entry);
    else solid.push(entry);
  }
  return { solid, attached, liquid };
}
...
/**
 * Схлопывает entries (solid ИЛИ liquid) в боксы, режет по лимиту объёма и
 * сортирует итоговые боксы по нижней Y (по возрастанию) - см. §10.4.8
 * BOT.md: твёрдые/жидкие боксы обязаны вставать снизу вверх, иначе то, что
 * должно быть опорой (например, камень под песком), может уйти позже
 * самого зависимого от неё блока, и тот на мгновение окажется без опоры и
 * упадёт. Array.prototype.sort в Node.js/V8 стабилен (гарантия ES2019),
 * поэтому при равной нижней Y бокса порядок остаётся как в исходном файле.
 * Возвращает и боксы ДО сортировки (в исходном порядке buildBoxes - нужны
 * для отладки), и splitBoxes ПОСЛЕ сортировки (нужны checkFallingSupport,
 * чтобы индекс в этом массиве совпадал с порядком реальной отправки).
 */
function buildSortedBoxCommands(entries, origin) {
  const boxes = buildBoxes(entries);
  // ВАЖНО: не boxes.flatMap(splitBox) напрямую - Array.flatMap зовёт колбэк
  // как (element, index, array), и index попадал бы в необязательный
  // параметр maxVolume у splitBox вместо MAX_FILL_VOLUME по умолчанию
  // (баг был в проде: боксы с малым индексом резались до отдельных клеток,
  // с большим - не резались вообще и могли превысить протокольный лимит
  // /fill в 32768 - см. BOT.md §5.2 "Опыт").
  const splitBoxes = boxes.flatMap((box) => splitBox(box));
  splitBoxes.sort((a, b) => a.y1 - b.y1);
  const commands = splitBoxes.map((box) => translateBox(box, origin)).map(boxToCommand);
  return { boxes, splitBoxes, commands };
}

/**
 * Строит индекс (x,y,z) -> позиция бокса в splitBoxes (уже отсортированном
 * по Y), чтобы checkFallingSupport могла сравнивать порядок отправки двух
 * произвольных клеток. Клетки внутри одного бокса получают один и тот же
 * индекс - они уходят одной командой, порядка между ними внутри команды
 * не существует (и не нужен - весь бокс встаёт одним /fill).
 */
function buildCellToBoxIndex(splitBoxes) {
  const index = new Map();
  splitBoxes.forEach((box, i) => {
    for (let x = box.x1; x <= box.x2; x++) {
      for (let y = box.y1; y <= box.y2; y++) {
        for (let z = box.z1; z <= box.z2; z++) {
          index.set(cellKey(x, y, z), i);
        }
      }
    }
  });
...
/**
 * Самопроверка порядка постройки (см. §12/§10.4.8 BOT.md): для каждой
 * падающей клетки схемы (FALLING_BLOCK_NAMES) смотрит клетку под ней.
 * - Если под ней в схеме вообще ничего нет - доверяем известному рельефу
 *   (§12), не нарушение.
 * - Если под ней жидкость или крепящийся блок - это не твёрдая опора,
 *   нарушение независимо от порядка боксов.
 * - Если под ней твёрдый блок (в т.ч. другой падающий) - его бокс должен
 *   идти строго раньше бокса самой падающей клетки (тот же бокс - тоже
 *   нормально: значит обе клетки уходят одной командой одновременно).
 * Возвращает массив строк с описанием нарушений (для лога).
 */
function checkFallingSupport(schema, solidSplitBoxes) {
  const schemaMap = new Map();
  for (const entry of schema) {
    schemaMap.set(cellKey(entry.x, entry.y, entry.z), entry);
  }
  const boxIndex = buildCellToBoxIndex(solidSplitBoxes);
  const warnings = [];

  for (const entry of schema) {
    const { name } = parseBlockSpec(entry.block);
    if (!isFallingBlock(name)) continue;

    const belowKey = cellKey(entry.x, entry.y - 1, entry.z);
    const below = schemaMap.get(belowKey);
    if (!below) continue; // опоры в схеме нет - считаем, что это известный рельеф

    const belowName = parseBlockSpec(below.block).name;
    if (isLiquidBlock(belowName) || isAttachedBlock(belowName)) {
      warnings.push(
        `(${entry.x},${entry.y},${entry.z}) ${name}: опора (${entry.x},${entry.y - 1},${entry.z}) ` +
        `- ${belowName}, не твёрдая`
      );
      continue;
    }

    const aboveIdx = boxIndex.get(cellKey(entry.x, entry.y, entry.z));
    const belowIdx = boxIndex.get(belowKey);
    if (aboveIdx === undefined || belowIdx === undefined) continue; // не должно происходить
    if (belowIdx !== aboveIdx && belowIdx > aboveIdx) {
      warnings.push(
        `(${entry.x},${entry.y},${entry.z}) ${name}: опора (${entry.x},${entry.y - 1},${entry.z}) ` +
        `встанет позже (бокс №${belowIdx} после бокса №${aboveIdx})`
      );
    }
  }
...
/**
 * Собирает итоговый план команд для схемы: делит на solid/attached/liquid
 * (см. splitSchemaByAttachment). Твёрдые и жидкие блоки жадно схлопываются
 * в боксы и сортируются по нижней Y (см. buildSortedBoxCommands) - твёрдые
 * идут первым проходом, жидкие - последним, ПОСЛЕ attached (см. §4.2/§4.4
 * BOT.md). attached блоки - как раньше, по одному, в порядке из файла,
 * между твёрдыми и жидкими. Дополнительно прогоняет самопроверку порядка
 * постройки для падающих блоков (см. checkFallingSupport).
 */
function buildFillCommands(schema, origin) {
  const { solid, attached, liquid } = splitSchemaByAttachment(schema);

  const solidResult = buildSortedBoxCommands(solid, origin);
  const singleCommandCount = solidResult.splitBoxes
    .filter((box) => box.x1 === box.x2 && box.y1 === box.y2 && box.z1 === box.z2)
    .length;

  const attachedCommands = buildAttachedCommands(attached, origin);

  const liquidResult = buildSortedBoxCommands(liquid, origin);

  const fallingSupportWarnings = checkFallingSupport(schema, solidResult.splitBoxes);

  return {
    commands: [...solidResult.commands, ...attachedCommands, ...liquidResult.commands],
    commandCount: solidResult.commands.length + attachedCommands.length + liquidResult.commands.length,
    boxCount: solidResult.boxes.length,
    solidCommandCount: solidResult.commands.length,
    singleCommandCount,
    attachedCount: attached.length,
    liquidCommandCount: liquidResult.commands.length,
    liquidBoxCount: liquidResult.boxes.length,
